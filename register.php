					...
					if($result -> num_rows > 0){
						while($row = $result -> fetch_assoc()){
							if($row["status"]=="active"){
								...
								$sql = "update user set fname ='$fname',email='$email',contact='$contact',password='$pass',lname='$lname',question='$question',answer='$answer',status='active'";
							}
						}
						echo "<script type='text/javascript'>location.href='login.php';</script>";
					}
					else{
						$sql = "insert into user values('$fname','$email','$contact','$pass','$lname','$question','$answer','active')";
